...
export default function TriplePendulumSimulation() {
  ...
  const energyRef = useRef(energy(initialState, params));

  useEffect(() => {
    stateRef.current = initialState;
    trailRef.current = [];
    energyRef.current = energy(initialState, params);
  }, [initialState, params]);
  ...
  const reset = () => {
    ...
    trailRef.current = [];
    energyRef.current = energy(stateRef.current, params);
  };

  return (
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100">
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[1.6fr_0.9fr]">
        <Card className="overflow-hidden rounded-3xl border-slate-800 bg-slate-900 shadow-2xl">
          <CardHeader className="border-b border-slate-800 pb-4">
            <CardTitle className="text-2xl font-semibold tracking-tight">Triple Pendulum Simulation</CardTitle>
            <p className="text-sm text-slate-400">Nonlinear RK4 model with trail rendering, damping, and live controls.</p>
          </CardHeader>
          <CardContent className="p-0">
            <canvas ref={canvasRef} className="h-[72vh] w-full" />
          </CardContent>
        </Card>
        ...
        <div className="grid gap-6">
          <Card className="rounded-3xl border-slate-800 bg-slate-900 shadow-xl">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm"><span>Top angle</span><span>{theta1.toFixed(2)} rad</span></div>
                <Slider value={[theta1]} min={-3.0} max={3.0} step={0.01} onValueChange={(v) => setTheta1(v[0])} />
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm"><span>Middle angle</span><span>{theta2.toFixed(2)} rad</span></div>
                <Slider value={[theta2]} min={-3.0} max={3.0} step={0.01} onValueChange={(v) => setTheta2(v[0])} />
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm"><span>Bottom angle</span><span>{theta3.toFixed(2)} rad</span></div>
                <Slider value={[theta3]} min={-3.0} max={3.0} step={0.01} onValueChange={(v) => setTheta3(v[0])} />
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm"><span>Damping</span><span>{damping.toFixed(3)}</span></div>
                <Slider value={[damping]} min={0} max={0.2} step={0.001} onValueChange={(v) => setDamping(v[0])} />
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm"><span>Simulation speed</span><span>{speed.toFixed(2)}x</span></div>
                <Slider value={[speed]} min={0.1} max={2.5} step={0.05} onValueChange={(v) => setSpeed(v[0])} />
              </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-slate-800 px-4 py-3">
                ...
                <Button variant={trailOn ? "default" : "secondary"} onClick={() => setTrailOn((v) => !v)} className="rounded-2xl">
                  {trailOn ? "On" : "Off"}
                </Button>
              </div>
            </CardContent>
          ...
          <Card className="rounded-3xl border-slate-800 bg-slate-900 shadow-xl">
            ...
            <CardContent className="space-y-3 text-sm text-slate-300">
              <p>This uses a nonlinear coupled model, so tiny changes in angle can produce visibly different motion.</p>
              <p>Higher damping makes the system settle. With very low damping, the motion remains energetic for much longer.</p>
              <p>Change the three initial angles, then press reset to relaunch from the new state.</p>
            </CardContent>
          </Card>
        </div>
      </div>
